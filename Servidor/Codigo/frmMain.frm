...
Select Case hyunpt
Case 53
Call SendData(ToAll, 0, 0, "||DeathMatch> En 10 minutos se realizará un deathmatch automatico." & FONTTYPE_GUILD)
Case 58
Call SendData(ToAll, 0, 0, "||DeathMatch> En 5 minutos se realizará un deathmatch automatico." & FONTTYPE_GUILD)
Case 62
Call SendData(ToAll, 0, 0, "||DeathMatch> En 1 minutos se realizará un deathmatch automatico." & FONTTYPE_GUILD)
Case 63
Call death_comienza(RandomNumber(8, 12))
Case 65
...
Case 89
Call SendData(ToAll, 0, 0, "||Torneo> En 2 minutos y 30 segundos se realizará un torneo automatico." & FONTTYPE_GUILD)
Case 93
Call SendData(ToAll, 0, 0, "||Torneo> En 30 segundos se realizará un torneo automatico." & FONTTYPE_GUILD)
Case 94
Call torneos_auto(RandomNumber(2, 4)) ' con esto se hace un random si el torneo sera de 4 a 16 participantes.
Case 96
...
For i = 1 To LastUser
 If Restando = 1 Then
  aa = aa - 1
   Call SendData(ToAll, 0, 0, "||El death comenzará en " & aa & "..." & FONTTYPE_INFO)
   If aa = 0 Then
   MapInfo(UserList(i).pos.Map).Pk = True
   Call SendData(ToAll, 0, 0, "||El death a comenzado" & FONTTYPE_FENIX)
   'Restando = 0
   aa = 0
   End If
   End If
If UserList(i).flags.EnDeath = 1 And Not Restando = 1 Then
aa = aa + 1
 
 
If aa = 15 Then
        Call SendData(ToAll, 0, 0, "||Faltan 45 segundos para que comienze el death." & FONTTYPE_TALK)
    ElseIf aa = 30 Then
    Call SendData(ToAll, 0, 0, "||Faltan 30 segundos para que comienze el death." & FONTTYPE_TALK)
    ElseIf aa = 45 Then
    Call SendData(ToAll, 0, 0, "||Faltan 15 segundos para que comienze el death." & FONTTYPE_TALK)
    ElseIf aa = 55 Then
  Restando = 1
  aa = 5
