...
If UserList(VictimIndex).pos.Map = 210 Then 'Cambien X por el numero de mapa donde ganan los puntos.
UserList(AttackerIndex).flags.Canje = UserList(AttackerIndex).flags.Canje + 1
UserList(VictimIndex).flags.Canje = UserList(VictimIndex).flags.Canje - 1
End If

If UserList(AttackerIndex).pos.Map <> 210 Then
...
Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & 35 & "," & 0) ' Animación
...
Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "CFX" & UserList(UserIndex).Char.CharIndex & "," & 2 & "," & 0) ' Animación
...
If UserList(UserIndex).flags.Privilegios Then
    bCr = 1
    ElseIf UserList(UserIndex).pos.Map = 195 Then
    bCr = 11
...
ElseIf EsNewbie(UserIndex) Then
    bCr = 4
ElseIf UserList(UserIndex).GuildInfo.FundoClan = 1 Then
    bCr = 6
Else: bCr = 5
End If
...
On Error GoTo ErrorHandler
If UserList(UserIndex).flags.EstaMision = True Then
Call SendData(ToIndex, UserIndex, 0, "||Mision> Perdiste la mision." & FONTTYPE_FENIX)
...
Dim pass As Long
    ...
    Call SendData(ToIndex, Ganador, 0, "||Has ganado, el password del oponente es " & pass & FONTTYPE_TALK)
        Call SendData(ToAll, 0, 0, "||El usuario " & UserList(Ganador).Name & "ha derrotado a  " & UserList(UserIndex).Name & " en un Reto por su PJ. ¡Disfrutelo!" & FONTTYPE_TALK)
...
If UserList(UserIndex).pos.Map = 214 Then
Call WarpUserChar(UserIndex, 214, 56, 37, True)
Call SendData(ToIndex, UserIndex, 0, "||Has sido teletransportado a Ullathorpe porque moriste Jajaja MANCO!" & FONTTYPE_TALK)
End If
...
If UserList(UserIndex).pos.Map <> 190 And MapInfo(UserList(UserIndex).pos.Map).SeCaenItems = 1 Then
    If Not EsNewbie(UserIndex) Then
...
'##################### GUERRACLAN by GALLE ###################
If UserList(UserIndex).flags.enRetoC1 Then
   
   If CLAN1 = C1 Then 'SI NUMERO DE MUERTOS DE UN CLAN = NUMERO DE JUGADORES DEL CLAN
      Call SendData(ToAll, 0, 0, "||Guerra de Clanes > El clan " & UserList(RetoClan.lider2).GuildInfo.GuildName & " ha ganado." & FONTTYPE_FENIX)
      For i = 1 To LastUser
          If UserList(i).ConnID > -1 And UserList(i).flags.enRetoC1 Or UserList(i).flags.enRetoC2 Then
   ...
   Else
      CLAN1 = CLAN1 + 1 'SUMAMOS
   End If
   
ElseIf UserList(UserIndex).flags.enRetoC2 Then
   
   If CLAN2 = C2 Then 'SI NUMERO DE MUERTOS DE UN CLAN = NUMERO DE JUGADORES DEL CLAN
      Call SendData(ToAll, 0, 0, "||Guerra de Clanes > El clan " & UserList(RetoClan.lider1).GuildInfo.GuildName & " ha ganado." & FONTTYPE_FENIX)
      For i = 1 To LastUser
          If UserList(i).ConnID > -1 And UserList(i).flags.enRetoC1 Or UserList(i).flags.enRetoC2 Then
             Call WarpUserChar(i, 1, 50, 50)
             UserList(i).flags.enRetoC1 = False
...
If UserList(UserIndex).pos.Map = 215 Then
If UserIndex = DesaFiante(2) Then
DeFenZas = DeFenZas + 1
If DeFenZas = 10 Then
DeFenZas = 0
Call SendData(ToAll, 0, 0, "||" & UserList(DesaFiante(1)).Name & " llegó a un total de 10 defenzas del titulo de campeón. Su premio ha sido entregado y la sala está libre otra vez" & FONTTYPE_TALK)
Call WarpUserChar(DesaFiante(1), 1, 50, 50, True)
Call SendData(ToIndex, DesaFiante(2), 0, "||Has llegado a las 10 defenzas del titulo y has ganado una suma de 1kk de monedas de oro" & FONTTYPE_TALK)
UserList(DesaFiante(1)).Stats.GLD = UserList(DesaFiante(1)).Stats.GLD + 1000000
Exit Sub
End If
Call SendData(ToAll, 0, 0, "||" & UserList(DesaFiante(1)).Name & " consigue una defenza del titulo de campeón. Con esta ya lleva " & DeFenZas & FONTTYPE_TALK)
Call WarpUserChar(DesaFiante(2), 1, 50, 50, True)
Call SendData(ToIndex, DesaFiante(2), 0, "||Has muerto y has sido llevado a Ulla" & FONTTYPE_TALK)
UserList(DesaFiante(2)).flags.Desafiando = False
End If
If UserIndex = DesaFiante(1) Then
DeFenZas = 0
UserList(DesaFiante(1)).flags.Esperando = False
Call SendData(ToAll, 0, 0, "||" & UserList(DesaFiante(1)).Name & " Perdió el titulo de campeón y ahora " & UserList(DesaFiante(2)).Name & " va por su primer defenza" & FONTTYPE_TALK)
Call WarpUserChar(DesaFiante(1), 1, 50, 50, True)
DesaFiante(2) = DesaFiante(1)
UserList(DesaFiante(1)).flags.Esperando = True
Call SendData(ToIndex, DesaFiante(2), 0, "||Has muerto y has sido llevado a Ulla" & FONTTYPE_TALK)
End If
End If

If UserList(UserIndex).pos.Map = 195 Then '
Call SendData(ToAll, 0, 0, "||DeathMatch>" & UserList(UserIndex).Name & " ha perdido." & FONTTYPE_GUILD)
Call WarpUserChar(UserIndex, 1, 50, 50, True)
...
If UserList(UserIndex).pos.Map = 195 Then ' iL Nolox!
Call SendData(ToAll, 0, 0, "||DeathMatch>" & UserList(UserIndex).Name & " se desconectó y por lo tanto ha perdido." & FONTTYPE_INFO)
