...
If InMapBounds(X, Y) Then
    ...
    If FoundChar = 1 Then
       ...
       If Len(UserList(TempCharIndex).GuildInfo.GuildName) > 0 Then
        ...
        ElseIf UserList(TempCharIndex).flags.Privilegios = 0 And UserList(TempCharIndex).pos.Map = 14 Then
        Stat = "PLAYER"
        Else
        ...
        Else
            If UserList(TempCharIndex).flags.Muerto Then
                Stat = "2K" & UserList(TempCharIndex).Name
                           ElseIf UserList(TempCharIndex).flags.Privilegios = 0 And UserList(TempCharIndex).pos.Map = 195 Then
                Stat = "B7" & Stat
