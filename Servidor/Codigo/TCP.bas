...
If UserList(UserIndex).flags.enRetoC1 Or UserList(UserIndex).flags.enRetoC2 Then 'GUERRACLAN
   Call WarpUserChar(UserIndex, 1, 50, 50) ' Si desloguea va ULLA
End If
 
 
 If UserList(UserIndex).flags.EnDuelos Then
    Call SendData(ToAll, 0, 0, "||Duelos> " & UserList(UserIndex).Name & " se ha desconectado en la Sala de Duelos." & FONTTYPE_TALK)
    Call WarpUserChar(UserIndex, 1, 50, 47, True)
...
Causa = GetVar(App.Path & "\logs\BanDetail.dat", Name, "Reason")
    For LoopC = 1 To Baneos.Count
        If Baneos(LoopC).Name = UCase$(Name) Then
            Call SendData(ToIndex, UserIndex, 0, "ERR" & Baneador & " te ha prohibido la entrada a WesterAo debido a tu mal comportamiento. La razón del ban es la siguiente: " & Causa & ". Tu personaje estará baneado hasta el día " & Format(Baneos(LoopC).FechaLiberacion, "dddddd") & " a las " & Format(Baneos(LoopC).FechaLiberacion, "hh:mm am/pm"))
            Exit Sub
            End If
    Next
    Call SendData(ToIndex, UserIndex, 0, "ERR" & Baneador & " te ha prohibido la entrada a WesterAo debido a tu mal comportamiento. La razón del ban es la siguiente: " & Causa & ". Tu personaje quedará baneado permanentemente.")
    Exit Sub
End If
...
MurioDeath = MurioDeath - 1
Call WarpUserChar(UserIndex, 1, 50, 50, True)
UserList(UserIndex).flags.EnDeath = 0
If MurioDeath = 1 Then
Call SendData(ToAll, 0, 0, "||Death finalizado, el ganador escriba /GANE, y recibirá su premio." & FONTTYPE_TALK)
End If
End If
   ...
   If Not UserList(UserIndex).flags.UserLogged Then
        ...
        Select Case Left$(rdata, 6)
            Case "BORRAR"
                ...
                '¿Está baneado?
                If BANCheck(Name) Then
                Call SendData(ToIndex, UserIndex, 0, "ERREl personaje se encuentra baneado y por lo tanto no se podrá borrar. Haga su descargo en el foro o contáctese con: darktester@WesterAo.com.ar. Muchas gracias por su atención.")
                Exit Sub
                End If
                ...
                Call SendData(ToIndex, UserIndex, 0, "ERREl personaje fué borrado exitósamente! Recuerde que una vez borrado NO será recuperado. Si el personaje se le fue robado y luego accedieron a este medio para borrarlo, contáctese con: darktester@WesterAo.com.ar. Muchas gracias.")
                Exit Sub
                End If
            Case "SARASA" '---------> OLOGIO
               ...
               Else
               Call SendData(ToIndex, UserIndex, 0, "!!El cliente es antiguo, por favor verifique la web www.WesterAo.es.tl y descargue el último parche de la versión " & UltimaVersion & " para poder conectarse al juego. Atte. WesterAo Staff.")
                     Exit Sub
                End If
            ...
            Case "RECUPE"
                ...
                If ComprobarCorreo(ReadField(1, rdata, Asc(",")), ReadField(2, rdata, Asc(","))) = True Then
                    If EnviarCorreo(ReadField(1, rdata, Asc(",")), ReadField(2, rdata, Asc(","))) Then
                        Call SendData(ToIndex, UserIndex, 0, "ERRUna nueva password fue generada. La nueva password ha sido enviada a su casilla de correo que ud. registró en el personaje. De usar hotmail, recuerde revisar su correo NO deseado. Atte. <WesterAo Staff>")
                    Else
                        Call SendData(ToIndex, UserIndex, 0, "ERRLo sentimos, pero la password no pudo ser reescrita. Inténtelo denuevo y si le salta denuevo este cartel contáctese con: darktester@WesterAo.com.ar")
                        Exit Sub
                    End If
                Else
...
Dim o As Obj
      ...
      If Not TieneObjetos(o.OBJIndex, 100, UserIndex) Then
      Call SendData(ToIndex, UserIndex, 0, "||No puedes hacerte V.I.P si no tienes 100 Copas De oro." & FONTTYPE_VIP)
      Exit Sub
      End If
     
      If UserList(UserIndex).Stats.GLD < 20000000 Then
      Call SendData(ToIndex, UserIndex, 0, "||No puedes hacerte V.I.P si no tienes 20.000.000 monedas de oro." & FONTTYPE_VIP)
      Exit Sub
      End If
     
      UserList(UserIndex).flags.VIP = 1
      UserList(UserIndex).Stats.MinHP = UserList(UserIndex).Stats.MinHP + 35
      UserList(UserIndex).Stats.MaxHP = UserList(UserIndex).Stats.MaxHP + 35
      UserList(UserIndex).Stats.MinMAN = UserList(UserIndex).Stats.MinMAN + 85
      UserList(UserIndex).Stats.MaxMAN = UserList(UserIndex).Stats.MaxMAN + 85
...
If HayDeath = 0 Then Exit Sub
If UserList(UserIndex).flags.Muerto = 1 Then Exit Sub
If UserList(UserIndex).flags.EnDeath = 1 Then Exit Sub
If CantidadDeath >= CupoMaximo Then
Call SendData(ToIndex, UserIndex, 0, "||Se lleno el death" & FONTTYPE_INFO)
Exit Sub
End If
...
MurioDeath = MurioDeath + 1
Call WarpUserChar(UserIndex, 195, 50, 55, True)
Call SendData(ToIndex, UserIndex, 0, "||Entraste al death." & FONTTYPE_INFO)
End If
 
If UCase$(rdata) = "/GANE" Then
If UserList(UserIndex).flags.EnDeath = 0 Then Exit Sub
If Not MurioDeath = 1 Then Exit Sub
 
For i = 1 To LastUser
If UserList(i).flags.EnDeath = 1 Then
 
Call SendData(ToAll, 0, 0, "||" & UserList(UserIndex).Name & " ganó el deathmatch." & FONTTYPE_INFO)
UserList(UserIndex).flags.EnDeath = 0
Restando = 0
Call WarpUserChar(UserIndex, 1, 50, 50, True)
HayDeath = 0
CantidadDeath = 0
frmMain.UserTimer.Enabled = False
End If
     Next i
     End If
...
'acepta
    ...
    Call WarpUserChar(UserIndex, 213, 11, 15, True) 'cambiar mapa y coordes del que manda
    Call WarpUserChar(TIndex, 213, 30, 72, True) 'cambiar mapa y coordes del que acepta
...
If UCase$(Left$(rdata, 8)) = "/CANJEO " Then
    ...
    If rdata = "T1" Then
        If UserList(UserIndex).flags.Canje >= 300 Then
        ...
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 300
        End If
        Exit Sub
    End If
    
    If rdata = "T2" Then
        If UserList(UserIndex).flags.Canje >= 150 Then
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 37 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 150
        End If
        Exit Sub
    End If
    
    If rdata = "T3" Then
        If UserList(UserIndex).flags.Canje >= 200 Then
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 38 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 250
        End If
        Exit Sub
    End If
    ...
    If rdata = "T6" Then
        If UserList(UserIndex).flags.Canje >= 320 Then
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 553 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 320
        End If
        Exit Sub
    End If
    ...
    If rdata = "T8" Then
        If UserList(UserIndex).flags.Canje >= 350 Then
        superoro.Amount = 1 'Cantidad de Items
    ...
    If rdata = "T9" Then
        If UserList(UserIndex).flags.Canje >= 250 Then
        superoro.Amount = 1 'Cantidad de Items
    ...
    If rdata = "T11" Then
        If UserList(UserIndex).flags.Canje >= 160 Then
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 716 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 160
        End If
        Exit Sub
    End If
    ...
    If rdata = "T16" Then
        If UserList(UserIndex).flags.Canje >= 400 Then
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 622 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 400
        End If
        Exit Sub
    End If
    ...
    If rdata = "T21" Then
        If UserList(UserIndex).flags.Canje >= 50 Then 'aca ponen el valor en puntos de canjes
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 407 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 50 'aca ponen los puntos que le sacan cuando canjean
        End If
        Exit Sub
    End If
    
    If rdata = "T22" Then
        If UserList(UserIndex).flags.Canje >= 4 Then 'aca ponen el valor en puntos de canjes
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 410 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 4 'aca ponen los puntos que le sacan cuando canjean
        End If
        Exit Sub
    End If
    
    If rdata = "T23" Then
        If UserList(UserIndex).flags.Canje >= 350 Then 'aca ponen el valor en puntos de canjes
        superoro.Amount = 1 'Cantidad de Items
        superoro.OBJIndex = 558 'Numero de Item
        If Not MeterItemEnInventario(UserIndex, superoro) Then Call TirarItemAlPiso(UserList(UserIndex).pos, superoro)
        Call SendData(ToIndex, UserIndex, 0, "||¡Has Obtenido un Item. Se te ha descontado de tus Puntos de Canje!" & FONTTYPE_INFO)
        UserList(UserIndex).flags.Canje = UserList(UserIndex).flags.Canje - 350 'aca ponen los puntos que le sacan cuando canjean
        End If
        Exit Sub
    End If
...
Call SendData(ToIndex, UserIndex, 0, "||Usted tiene un total de " & UserList(UserIndex).Stats.Advertencias & " Advertencias. Te quedan " & (6 - UserList(UserIndex).Stats.Advertencias) & " más y serás baneado si te comportas mal.!" & FONTTYPE_GUILD)
Exit Sub
End If

If UCase$(Left$(rdata, 8)) = "/MISION " Then ' BurnS
    rdata = Right$(rdata, Len(rdata) - 8)
   
    If rdata = "M1" Then
        Call SendData(ToIndex, UserIndex, 0, "||¡Tú misión es la siguiente : debes matar 10 Usuarios!" & FONTTYPE_VENENO)
        Call SendData(ToIndex, UserIndex, 0, "||¡Cuando acabes con ella Tipea /Recompensa!" & FONTTYPE_VENENO)
        UserList(UserIndex).flags.EstaMision = True
        Exit Sub
    End If
   
        If rdata = "M2" Then
        Call SendData(ToIndex, UserIndex, 0, "||¡Tú misión es la siguiente : debes matar 20 Usuarios!" & FONTTYPE_VENENO)
                Call SendData(ToIndex, UserIndex, 0, "||¡Cuando acabes con ella Tipea /RecompensaD !" & FONTTYPE_VENENO)
        UserList(UserIndex).flags.EstaMision = True
        Exit Sub
    End If
   
        If rdata = "M3" Then
        Call SendData(ToIndex, UserIndex, 0, "||¡Tú misión es la siguiente : debes matar 40 Usuarios!" & FONTTYPE_VENENO)
                Call SendData(ToIndex, UserIndex, 0, "||¡Cuando acabes con ella Tipea /RecompensaT!" & FONTTYPE_VENENO)
        UserList(UserIndex).flags.EstaMision = True
        Exit Sub
    End If
   
    If rdata = "M4" Then
        Call SendData(ToIndex, UserIndex, 0, "||¡Tú misión es la siguiente : debes matar 50 Usuarios!" & FONTTYPE_VENENO)
                Call SendData(ToIndex, UserIndex, 0, "||¡Cuando acabes con ella tipea /RecompensaC!" & FONTTYPE_VENENO)
        UserList(UserIndex).flags.EstaMision = True
        Exit Sub
    End If
    Exit Sub
...
If UCase$(Left$(rdata, 8)) = "/ACTCLAN" Then
   ...
   If cACT = False Then
      cACT = True
      Call SendData(ToAll, 0, 0, "||Guerra de Clanes Activada." & FONTTYPE_FENIX)
   Else
      C1 = 0
      C2 = 0
      CLAN1 = 0
      ...
      RetoClan.lider2 = 0
         Dim LoopCla As Integer
               For LoopCla = 1 To LastUser
                  If UserList(LoopCla).ConnID > -1 And UserList(LoopCla).flags.enRetoC1 Or UserList(LoopCla).flags.enRetoC2 Then
      ...
      Call SendData(ToAll, 0, 0, "||Guerra de Clanes Desactivada." & FONTTYPE_FENIX)
   End If
Exit Sub
End If
'########################## GUERRACLAN by GALLE ########################
...
spll.Amount = 1  'Cantidad de copas
     spll.OBJIndex = 410  'Numero de item
...
If UCase$(UserList(UserIndex).Name) <> "DarkTester" Then Exit Sub 'QUIEN LO USA
