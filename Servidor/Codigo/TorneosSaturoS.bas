...
End If
    ...
    If (Torneo_Rondas = 1) Then
        ...
        Call SendData(ToAll, 0, 0, "||PREMIO: 20 Puntos de Canjeo y 1 Punto de Torneo." & FONTTYPE_GUILD)
        UserList(Torneo_Luchadores(LI1)).flags.Canje = UserList(Torneo_Luchadores(LI1)).flags.Canje + 20
...
If (Torneo_Activo) Then
                ...
                Exit Sub
        End If
        Call SendData(ToAll, 0, 0, "||Torneo: Esta empezando un nuevo torneo 1v1 de " & val(2 ^ rondas) & " participantes!! para participar pon /ENTRAR - (Cae inventario)" & FONTTYPE_GUILD)
...
Dim UI1 As Integer, UI2 As Integer
    ...
    Call SendData(ToAll, 0, 0, "||Torneo: " & UserList(UI1).Name & " versus " & UserList(UI2).Name & ". Esquinas!! Peleen!" & FONTTYPE_GUILD)
