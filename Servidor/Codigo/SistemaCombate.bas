...
Else
    ...
    Call SendData(ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "||" & vbCyan & "Falla" & "°" & str(Npclist(NpcIndex).Char.CharIndex))
End If
...
If MapData(UserList(VictimIndex).pos.Map, UserList(VictimIndex).pos.X, UserList(VictimIndex).pos.Y).trigger = 6 And MapData(UserList(AttackerIndex).pos.Map, UserList(AttackerIndex).pos.X, UserList(AttackerIndex).pos.Y).trigger = 6 Then
Call SendData(ToIndex, AttackerIndex, 0, "SI")
    PuedeAtacar = True
    Exit Function
End If
...
If UserList(VictimIndex).pos.Map = 190 Or UserList(VictimIndex).pos.Map = 192 Or UserList(VictimIndex).pos.Map = 194 Or UserList(VictimIndex).pos.Map = 195 Or UserList(VictimIndex).pos.Map = 196 Or UserList(VictimIndex).pos.Map = 198 Or UserList(VictimIndex).pos.Map = 199 Or UserList(VictimIndex).pos.Map = 200 Then
    PuedeAtacar = True
    Exit Function
End If
